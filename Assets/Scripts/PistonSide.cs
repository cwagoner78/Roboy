...
public class PistonSide : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        Vector2 direction = (other.transform.position - other.transform.position).normalized;
        ...
        if (triggeredByPlayer)
        {
            ...
            player.rb.AddForce(direction * pistonForce);
            source.pitch = 1;
            source.Play();
        ...
        {
            transform.position = new Vector2(transform.position.x, transform.position.y + 0.5f);
            prize.rb.AddForce(direction * pistonForce);
            source.pitch = 1.5f;
            source.Play();
            StartCoroutine(Timer());
        }
